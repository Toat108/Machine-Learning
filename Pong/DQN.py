...
class ReplayBuffer:
    ...
    def push(self, state, action, reward, next_state, done):
        state = np.array(state, dtype=np.float32) # データ型をfloat32(float64では計算コストが高い)、またarrayで統一
        next_state = np.array(next_state, dtype=np.float32)

        if len(self.buffer) < self.capacity:
            self.buffer.append(None) #バッファの容量が足りないときは空のスロットを追加する
        self.buffer[self.position] = (state, action, reward, next_state, done) #諸変数を記録する
    ...
    def sample(self, batch_size):
        ...
        return (
            ...
            #ベクトル量は一度tensor化させることでリスト内に個々のテンソルを作成、そしてstackにより1つのテンソルとして結合(NNに入力するため)
